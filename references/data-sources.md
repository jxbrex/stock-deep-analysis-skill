...
| 北交所 (92xxxx) | `0.` | `0.920982`（83/43 等旧码 2025-10 起已切换 920 段，东财对旧码返回零值陈旧档，勿再用 `0.830799` 之类旧示例；脚本侧 `_em_quote` 会拦截旧码报错） |
...
- **现金含量**：HKF10 可得经营现金流（`NETCASH_OPERATE`）时须手工算（经营现金流÷净利）；
  判定口径与落 △ 纪律（含妙想 MCP 第二路径）以 `scoring.md`「1D 盈利质量红旗清单·港股豁免」为准
...
## 限流预算（tushare / 东财 / 妙想三通道调用约束）

各通道的调用频次约束与缓存现状（写实然；未公开的数字标「待实测」）：

### tushare（em_fetch `ts_call()` HTTP 通道——首选通道）

- **代码内无显式节流 sleep**——防超额不靠限速，靠缓存把网络请求压到最小：
...
- **无权限接口清单（当前会员包）**：`news`（新闻快讯）、`hk_income`/`hk_fina_indicator`（港股财务）、
  `yc_cb`、`cyq_*`（筹码分布）——脚本/文档已内置降级：港股财务 → 东财 HKF10 兜底 → 妙想
  `mx_hk_finance_data`；risk_free → WebSearch「中国 10 年期国债」降级；筹码 → 妙想 `mx_ashare_finance_data`。
...
- 免鉴权但反爬：push2 域对 urllib TLS 指纹间歇限流 → 脚本 **curl 优先、urllib 兜底**；
  HTTP 429/5xx → `RateLimitError` **硬停不重试**（main 退出码 2）；网络层错误（超时/连接失败）
  才重试 1 次（间隔 1.5s）。
- **并发克制**：`summarize()` 内 `ThreadPoolExecutor(max_workers=4)`（注释：避免触发限流）；
  E1 先行（K 线缩放校准依赖 E1 报价），peers 各股进程内**串行**（每股内部 4 并发）。
  磁盘缓存：push2/push2his 2h、datacenter 12h。
...
2. **禁止继续在同一路径堆调用**——第一次 429 后继续在同一路径硬跑十几分钟到第二次
   429 才停，是严格禁止的"硬挺"
