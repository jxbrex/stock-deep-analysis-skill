...
- [ ] v4.11.3 三卡字段是否已填：`drivers`/`driver_verdict`（P0 驱动卡，第一变量与 sensitivity 首行同名）、`cycle_stages`（周期章，driver 显示宽 ≤48、current 恰 1 个）、`dcf`（稳健成长/稳定价值强制，verdict ≥40 字、现价比价不手写由脚本算）？三种旧手写写法（p0_html「为什么…」info-card / cycle_html 阶段表 / valuation_html DCF 表）是否已清除？
