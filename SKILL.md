...
③成本端改善（量驱动下行期缩量提效，非涨价所得）——归因结论进 3.3 论据与 3.4 判词
...
2. **渲染**：
   ...
   **估值分按 `valuation_inputs` 四件套脚本强制计算，fill 手填值一律被覆盖**）、
   ...
   侧栏目录与全部 SVG 图表（目标价走廊/评分分布/同业散点恒生成；财务趋势图墙 `fin_trend`（3.4）、
   利润增长图 `growth_plot`（4.1）为 v4.9 必填图字段——缺失直接拒渲染，不静默跳过（数据照抄 E3 年表 /
   ...
   走廊卖方目标价带 `consensus`、触发条件状态条 `triggers`（13 章仪表盘前）按可选字段存在性生成，缺失静默跳过；
   PE 历史带 `pe_history` 与股价季K图挂在第 10 章——`cycle_html` 缺失时整章连同两图一起消失，渲染器告警；占位符替换、
...
- [ ] 3.3 护城河是否用 trig-strip 清单列示（2-4 条：名称 + 一句论据 + 评级 宽阔/中等/狭窄，hit/pending/miss 三色）？4.3 催化剂是否同款清单（事件 + 时间窗 + 影响，已发生/进行中/已证伪）？4.2 多项目标的是否考虑同款清单（项目 + 产能/投产时间 + 进度；单项目可整段论证）？清单块 `<p>` 是否 ≤2（判词+评分段，渲染器超限告警）？
- [ ] 3.3 新三约束是否落实（v4.10.1）：定价权类论据带分域可观测判据（B2B/2C/金融，无判据按空话顶格 5.9，禁跨域套用）？边际改善类论据（毛利率回升/不良下降/份额提升）带同步性检验 α/β 结论（变窄判据同样过检验）？评分末拍竞争压力测试四件套齐全（场景/判断/先例/阈值，阈值在 13 章 triggers/dash 有对应行）？周期股毛利率归因三方向（产品端/价差/成本端）已做？
...
- [ ] v4.9 必填图字段是否已填：`fin_trend`（3.4 财务趋势图墙）+ 锚点 `<!--FIN_TREND-->` 写在 3.4 维度块内、`growth_plot`（4.1 利润增长图）+ 锚点 `<!--GROWTH-->` 写在 4.1 维度块首——缺失即拒渲染（结构性下限同样拒：fin_trend 年数 ≥3 且有效面板 ≥3、growth_plot hist 有效年 ≥3；`growth_plot` 未盈利/管线分型豁免；数据分别照抄 E3 年表与 E5 一致预期，禁手写年表/手估）？`peers_plot` 是否已填（缺失触发渲染器告警，消解方式见「报告产出格式」第 2 步）？目标点与同业是否同口径（A/H、IFRS/经调整）？可选图字段是否已按数据可得性填写（缺失对应图不生成、不告警）：`sensitivity`（2 章龙卷风）；`segments`（3.1 业务构成，E6 回填、**利润口径=毛利**）+ 锚点 `<!--SEGMENTS-->` 是否写在 3.1 维度块内；`industry_chain`（3.2 产业链，只列行业不列企业）+ 锚点 `<!--CHAIN-->`；`consensus`（7 章走廊卖方目标价带，E5 回填）；`triggers`（13 章触发条件状态条，status 限 hit/miss/pending、宜 3-6 条）；`holders`（11 章户数趋势，v4.11.0 起照抄 E4「季度序列」行 ≥3 期）；`pe_history` 是否按 v4.7.1 分型规则处理（周期/稳健成长/稳定价值/困境反转 → 第 10 章必写 + 本字段按数据可得性填，含 `milestones` 与 v4.8 `p25`/`p75`（E1 落盘直接引用，禁手估）；快速成长/未盈利 → 禁填）？`price_history` 是否同规则处理（v4.11.0 起照抄 E2「全序列」行的 open/high/low/close/pe 四值+月末 PE（`YYYY-MM=开,高,低,收[/PE]`；四值为 ? 的月只写 close/pe），季K 蜡烛由脚本聚合、亏损期底纹与 PE 截断自动处理；`cycle_html` 缺失时两图无处显示会告警）？
