...
2. **渲染**：
   ...
   **估值分按 `valuation_inputs` 四件套脚本强制计算，fill 手填值一律被覆盖**）、
   ...
   PE 历史带 `pe_history`、回测哑铃 `prev` 按可选字段存在性生成，缺失静默跳过）、占位符替换、
...
- [ ] `peers_plot` 是否已填（缺失则图静默跳过）？目标点与同业是否同口径（A/H、IFRS/经调整）？可选图字段 `sensitivity`/`pe_history` 是否已按数据可得性填写（缺失对应图不生成、不告警）？
