...
def find_prev_report(code: str, directory: str = ".", include_unmarked: bool = False) -> str:
    ...
    for fn in os.listdir(directory):
        ...
        cands.append((m.group(1), marked, p))
    if not cands:
        return None
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "--find":
        ...
        if not code:
            print("用法: python extract_review.py --find [代码] [--include-unmarked]",
                  file=sys.stderr)
            sys.exit(1)
        prev = find_prev_report(code, include_unmarked="--include-unmarked" in sys.argv)
        if not prev:
            print(f"工作目录未找到 {code} 的脚本生成旧报告 → 正常首次分析，不进入回测模式",
                  file=sys.stderr)
            sys.exit(0)
