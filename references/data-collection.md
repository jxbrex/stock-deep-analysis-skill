...
公告 PDF 下载并提取指定表格。
**可下放**：新闻/公告搜集初筛（只列标题+摘要+链接）；复盘旧报告的假设/触发条件提取
（优先 `scripts/extract_review.py`，见 backtest.md）；fill JSON 对照 fill-schema 的自检。
**绝不下放**：定性调研、分型判断、评分打分、三情景假设、判词与预期差拆解——这些是弱模型的塌陷区，
下放等于把质量命门交出去。
