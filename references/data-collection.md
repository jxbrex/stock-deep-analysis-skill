...
**12 章时机判定小表的技术面信号一律取自落盘 `timing`，禁止手估**（神华假 MA60 同源修复））、
...
E2 月线区间 + 「全序列」行（前复权 OHLC + 月末 PE(TTM)，回填 `price_history` 图字段，v4.11.0 季K 化）、E4 股东户数 + 「季度序列」行（季末近 12 期，回填 `holders` 图字段，v4.11.0 起）、业绩预告/快报、1E 治理包（质押/增减持/回购）、
E5 一致预期+目标价（目标价区间回填 `consensus` 图字段）、
E6 主营构成（含**毛利额与毛利占比**，回填 `segments` 图字段；分部净利润无公开披露，口径=毛利；
同收入同成本的重复条目脚本已去重），含全部 peer。
**禁止给 em_fetch 加 `| tail` / `| head` 等截断管道**——`| tail` 会砍掉输出头部，而头部正是
目标公司本体数据、尾部只剩同业（截断 = 丢本体留同业，还得重跑一遍）。
输出本就是紧凑摘要（约 3.5KB/股 + v4.11.0 起 E2 全序列/E4 季度序列约 +2KB），直接进上下文是安全的。

**委派边界（两次截断丢本体后立规；细则权威在 Step 1.3，本处只留执行要点）**：
- **目标公司 E1 一律主会话自跑**，并加 `--out=_em_[代码]_quote.json` 落盘（fill 的
  `quote.source_file` 引用它，渲染器比对防伪）；子代理只处理 peers 部分。
  落盘文件在渲染成功后由 render 自动归档至报告目录 `_archive/`（v4.10 起），
  提前移动/删除 = 防伪链断裂拒渲染。
- **禁止指示子代理删除输出文件**；临时文件由主会话确认回传完整后再清理。
- **回传截断 / 输出文件不存在 / 任一本体数据缺失 → 必须重跑 em_fetch**；禁止凭记忆、
...
等搜索引擎结果页。** 搜索引擎结果页是低质、反爬、且需要二次解析的最差数据源。
...
2. **禁止在 Bash 里内联 `python -c "..."` 传递含中文的代码**。Windows shell 对内联中文
   支持不可靠，中文会被静默吞掉（实证：连续 3 次内联中文调 E7 全部无输出）。
...
8. **禁止静默降级**：任何降级必须同时满足 ① 已尝试 ≥2 种获取路径 ② 报告中明确标注降级原因。**禁止把"未检查"伪装成"通过"**——如"✓ API未触发"这类写法（芯原股份实证：应收/存货周转数据缺失被标成 ✓，读者误以为检查通过）。数据缺失时的三态标注（✓/✗/△）口径见 scoring.md「1D 盈利质量红旗清单」。
...
## Step 1.3：委派子代理（条件强制，委派边界一条规则）

**委派边界（本处为细则权威，SKILL.md Phase 1 只留骨架）**：

- **目标公司本体与定性调研：一律主会话，禁委派**——目标公司 E1 主会话自跑并加
  `--out=_em_[代码]_quote.json` 落盘（fill 的 `quote.source_file` 引用、渲染器比对防伪，
  见 Step 1.0）；定性调研在主会话完成——子代理脱离技能上下文会退化为抓搜索页（见上方纪律）。
- **同业 peer ≥3 只：批量取数与清洗成表必须委派（MANDATORY）**——子代理只处理 **peers 部分**：
  抓取 + 清洗成表回传，回传**必须含每个 peer 的 E1 原文行**，不足即视为委派失败——重跑，不凑合。
  em_fetch 已是单命令批量 + 进程/磁盘缓存，委派目的 = **把 peers 大块原始输出剥离出主会话上下文、
  由子代理压缩回传**（token 大户），不是脚本跑不动；em_fetch 一条命令会连带输出目标公司本体——
  委派出去的命令**只准回传 peers 部分**，禁止把目标公司内容交给子代理回传/改写。
- **peer <3 只：主会话直接跑，不必委派**（委派协商成本高于收益）。
- **委派期间主会话不空等**：目标公司本体取数、妙想 MCP 检索、定性调研与 peers 委派互不依赖，
  同一回合并行推进（见 SKILL.md Phase 1「并行发起」）。

公告 PDF 下载提表（纯机械、无本体风险）同样**必须委派**；其余委派仅允许边界清晰的**机械子任务**
（抓取指定 URL 列表的结构化字段），且 prompt 必须逐字写入工具限制：
...
**回传纪律**：子智能体只准回传**结构化原文数据**（数字+来源），禁止回传"总结"；
主模型核对来源后才进 fill。

**可下放（推荐，均为纯采集/机械修复）**：
- 妙想 MCP 检索类取数（mx_finance_search_news 研报观点 / mx_finance_search_notice 公告 /
  mx_macro_data 商品与宏观 / mx_ashare·mx_hk 补数）——单次返回块大、纯机械采集。
  回传模板逐字钉死：只准逐条「原文引文/数字 + 来源 + 日期」列表；**禁止综述句式**
  （"综合来看""总体而言"），禁止评价、建议与二次加工——定性判断仍由主会话完成。
- 新闻/公告搜集初筛（只列标题+摘要+链接）；复盘旧报告的假设/触发条件提取
  （优先 `scripts/extract_review.py`，见 backtest.md）。
- render `--check` 修复循环的**契约层**修复：JSON 语法/缺字段/键名/引号类报错，
  以退出码 0 为终点。**内容地板类报错（dim-block 薄块、thesis 信息量、conclusion 字数不足）
  必须回主会话补写**——那是分析不是修复（万华 2026-08-26 实证：5 轮预检迭代含契约与
  内容两类修复，内容类均由主会话完成）。
**绝不下放**：定性调研、分型判断、评分打分、三情景假设、判词与预期差拆解、内容地板补写——
这些是弱模型的塌陷区，下放等于把质量命门交出去。
