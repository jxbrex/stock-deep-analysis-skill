...
**截断事故与委派边界（新华保险 `| tail`、神华 601088 委派回传截断，两次同类翻车后立规）**：
- **目标公司 E1 一律主会话自跑**，并加 `--out=_em_[代码]_quote.json` 落盘（fill 的
  `quote.source_file` 引用它，渲染器比对防伪）；**子代理只跑 peers**，且回传必须包含
  各 peer 的 E1 原文行，不足即视为委派失败——重跑，不凑合。
...
## Step 1.3：委派子代理（条件强制，定性调研禁止委派）

**定性调研一律在主会话内完成**（子代理脱离技能上下文会退化为抓搜索页，见上方纪律）。
**条件强制委派（MANDATORY）**：同业 peer **≥3 只**的 em_fetch 批量取数、公告 PDF 下载提表，
这两类**必须**下放子智能体（token 大户且纯机械，主会话自己逐条跑是主要浪费）；
peer <3 只时主会话直接跑，不必委派。其余委派仅允许边界清晰的**机械子任务**
...
**必须下放（条件强制）**：同业 peer ≥3 只时的 em_fetch 批量取数与清洗成表；
公告 PDF 下载并提取指定表格。
**可下放（推荐，均为纯采集/机械修复）**：
