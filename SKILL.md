...
## 指导思想（v5.1.0 起，用户拍板固化）

本技能的第一性是**价值投资**：市场先生负责报价，不负责定价——内在价值锚独立于
即时价格；安全边际=价值−价格，价值随价动则安全边际恒失效。由此定三轨分工：

- **评分（质量/估值/时机）是主观的横向对比工具**：分数值不追求客观精确，职责是在
  同类型企业间给出直观的高低排序。评分的硬通货是**逻辑自洽**（同一把尺子量到底，
  禁止评分章与估值章各用一尺）与**信息完整**（优势与风险如实呈现），不是分数本身。
- **估值是交易决策的决定性变量**：不承诺数字准确（估值是假设的函数），承诺
  **假设可追溯、口径前后一致、锚不滑动**；估值以范围表达，靠回测持续精进。
- 落地纪律=估值锚四条（增量证据锁死 PE 带 / 回滚条款前置 / 锚移动台账 /
  尺子一致性声明），细则见 `references/scoring.md`「估值锚纪律」节，渲染器硬校验执行。
...
- **文件命名由脚本完成**：`{公司名}-{代码}-{质量分}-{估值分}-{日期}.html`
  ...
  模型不要自己命名文件。
- **归档目录 `_archive/`**（v4.10 起）：渲染成功后 fill 与 E1 防伪落盘由脚本自动移入
  报告目录下 `_archive/`（改名含公司名/代码/日期），并追加索引 `_archive/_index.jsonl`——
  跨报告对比与评分校准的数据源；不再删除。
...
- 回测触发机械判定：先跑 `python "<技能目录>/scripts/extract_review.py" --find [代码]`——代码可带
  `.SH/.SZ/.HK/.BJ` 后缀或 `sh/sz/hk/bj` 前缀（脚本自动归一），报告不在当前目录时加 `--dir <路径>`
  （递归子目录，不扫 artifacts/ 与 `_`/`.` 开头目录）。
...
"X 变 10% → 利润约变 Y%"，选弹性最大的 1-2 个。**弹性估算必须可复算（v5.1.2 估算纪律，
影石 impact=100 虚高实证）**：三要素齐全——变动额（净利约±X亿）、净利基数（以哪个年度的
净利为分母，全表统一）、边际假设（费用刚性/边际净利率口径一句）；**增量收入不增利**
（边际净利率 ≤0，影石 2026H1 实证 −26%）时禁用历史净利率外推，必须用边际口径；
**同一因果链只留一个刻度**（「存储芯片价格 ±20%」与「毛利率（存储成本传导）±1pct」是
同一条链的两个刻度，重复计量会把加冕排序顶走）。敏感性写入 fill 的 `sensitivity` 字段
（变动幅度/净利影响%/金额影响——**变动幅度须带量纲（v5.1.1）：比例写 ±10%、百分点写 ±1pct**，
净利影响 % 由脚本按 impact 生成），渲染器生成龙卷风图——写了字段就不必在 P0 正文手写敏感性表（图替代表）。
驱动卡写入 fill 的 `drivers`/`driver_verdict` 字段（v4.11.3 起，渲染器生成弹性卡 + 三情景锚 + 判词横条）——
P0 正文不再手写「为什么 X 是第一变量」info-card（判词横条由字段承载，契约见 fill-schema）。
**第一变量不再以手标为准（v5.1.1）**：脚本按 `sensitivity` |impact| 降序加冕，龙卷风图首行、
驱动卡角标与判词前缀同源；降序首行必须有同名驱动卡，否则拒渲染（`first_var` 仅在缺 sensitivity 时回退生效）。
**三情景必须围绕这 1-2 个驱动定义，而不是拍利润区间。**

**周期行业追加（剪刀差归因，v4.10.1）**：标的属周期行业（Phase 0.5 分型表枚举）时，
须把最新一期毛利率变动归因三方向之一——①产品端扩张（价驱动）②价差变动（价差驱动）
③成本端改善（量驱动下行期缩量提效，非涨价所得）——归因结论进 4.3 论据与 4.4 判词
（防把缩量提效当涨价、把价差收窄掩盖在收入增长里）。口径见 scoring.md 1C「周期股归因」。
...
| **稳定价值/金融** | 盈利稳定、高分红、杠杆经营（银行、保险、公用事业、高速公路） | **85:15**（增长不重要） | **PB-ROE 框架**（银行；保险/券商主锚见 industry-financials.md）；DDM（高股息公用事业，税后股息）；非金融加 DCF 强制三行（scoring.md「DCF」节）。PE 仅作辅助 | 1D 资产质量、1E 治理、1F 分红可持续性 |
| **稳健成长股** | 利润增速 10-25%、可预测性强（消费、医药白马、制造业龙头） | 70:30（默认） | PE 匹配 + **DCF 强制三行**（保守 DCF 值/现价隐含永续增速/base rate 对照，v4.9 起不看 FCF 门槛）+ PEG 辅助 | 1C 护城河、3A 增长持续性、1F 资本回报 |
...
- **强制委派（MANDATORY）**：同业 peer **≥3 只**的 `--peers` 批量取数、公告 PDF 下载提表
  **必须委派子代理**（回传须含各 peer 的 E1 原文行 + 清洗成表，主会话只核对来源）。
  em_fetch 已单命令批量+进程/磁盘缓存，委派目的=**把 peers 大块原始输出剥离出主上下文、
  由子代理压缩回传**，不是脚本跑不动；peer <3 只时主会话直接跑，不必委派。
- **禁委派**：目标公司本体取数（E1 加 `--out` 落盘供渲染器防伪）与定性调研一律主会话完成
  （细则见 data-collection.md Step 1.0「委派边界」+ Step 1.3）；em_fetch 一条命令会连带输出
  目标公司本体——委派出去的命令**只准回传 peers 部分**，禁止让子代理做定性。
- **可委派（推荐）**：妙想 MCP 检索类取数（研报观点/公告/商品宏观，只回传引文+出处+日期）、
  `--check` 修复循环的契约层修复。
**并行发起（三者无依赖）**：目标公司本体取数、妙想 MCP 检索、目标公司定性调研与子代理的
peers 批量取数互不依赖——委派发出后主会话立即并行推进自己的那几路，不空等子代理。

**工具检查硬门禁（开始取数前执行）**：检查工具列表是否含 `mcp__mx-ds-mcp-stdio__mx_*`（妙想 MCP）；
缺席再查 skill 列表是否含 `mx-data`/`mx-search`（妙想 skill 替补，同一东财妙想 API，经 Skill 工具调用——
定量查数用 `mx-data`、定性检索用 `mx-search`、选股筛选用 `mx-xuangu`）。两级均缺席 → 定性检索/筹码分布/
行业估值水位走降级链（见 data-collection.md），且**必须在报告显式标注「妙想 MCP 缺席，已走降级路径」**；
经 skill 替补的标注「妙想经 mx-data/mx-search skill 通道」——禁止静默跳过
（缺席未标注 = 违反降级纪律，见文末「数据降级与反幻觉」）。
...
连续2个季度以上下滑、之后又恢复；③ 行业属周期行业（与 Phase 0.5 分型表同枚举：
大宗商品/化工/航运/养殖/半导体/面板/钢铁/煤炭/有色）。
非周期股（Phase 0.5）默认跳过。
执行：阶段拆解写 `cycle_stages` 字段（v4.11.3 起，渲染器生成步骤卡——日期区间/股价范围/PE范围/驱动因素 显示宽 ≤48（CJK 计 2、ASCII 计 1），
手写阶段表废止）+ 当前位置判断 + 提取 3-5 条可复用规律（cycle_html 承载）。
...
**决策矩阵全表（9 行）只在 `references/scoring.md`，报告中不再出现**——报告第 12 章只显示落位与结论，
...
回测触发与输出差异全部在那里。触发：`extract_review.py --find [代码]` 在扫描目录（默认工作目录，`--dir` 可指定，递归子目录）找到同代码旧报告
（脚本判定，含渲染器标记验证）→ 自动进入回测模式
（先独立取数打分 → 再读旧报告 → 全量重写 → 最后新旧对比）。复盘内容只进 R 章节，
**禁止生成独立 `_复盘.md` 文件**。
**v5.1.0 起回测模式新增两件照抄/必填**：`prev.scenarios` 照抄 extract_review 输出的
scenarios 数组原文（锚移动台账与 PE 带移动校验的共同数据源，禁手改）；基础情景 PE 带
相对上版移动时 `valuation.pe_band_evidence` 必填增量证据清单（至少一条「基本面」类），
移动无理由则锁回上版数值——估值锚纪律见 `references/scoring.md`「估值锚纪律」节。

### Phase 7: 拼装成稿

**⚠️ 红灯熔断检查（输出报告前执行，对应第 6 章红灯层）：**
...
2. **渲染**：
   ...
   **估值分按 `valuation_inputs` 四件套脚本强制计算，fill 手填值一律被覆盖**）、
   徽章色、三轨判词、决策矩阵落位与「三轨判定与仓位结论」卡、质量分汇总章（第 7 章，只含质量轨明细）、
   侧栏目录与全部 SVG 图表（目标价走廊/评分分布/同业散点恒生成；财务趋势图墙 `fin_trend`（4.4）、
   利润增长图 `growth_plot`（5.1）为 v4.9 必填图字段——缺失直接拒渲染，不静默跳过（数据照抄 E3 年表 /
   ...
   **渲染后检查警告**：若输出含 `⚠️ fill JSON 缺图形字段`，交付前必须逐条消解（闸门分级以
   fill-schema.md 为准）：`valuation`（或兜底 `scenarios`）缺失 → 必须修复重渲，无豁免；
   `peers_plot` 缺失 → 二选一：补字段重渲，或确属同业数据凑不齐时改在 `peers_html` 手写
   matrix-table 兜底（此时渲染器不再告警）。
   **渲染后视觉审计（v4.11.3）**：渲染成功后跑
   `python "<技能目录>/scripts/visual_check.py" "<报告.html>"`——双视口（1280/390）查横向溢出/
   单行槽位折行/裁切，有问题逐条消解（槽位折行=缩短文案到契约上限内，上限见 fill-schema
   「槽位契约」节；长 section-meta 把章标题挤折行是已修复形态，新模板不再出现）；
   未装 Chrome/Edge 时脚本自动跳过、不阻塞交付。
3. **完成后（归档由脚本自动完成，v4.10 起不再删除）**：render 成功即把 `_fill_[代码].json`
   与 `quote.source_file` 引用的 E1 落盘移入报告目录 `_archive/`（改名
   `fill_{公司名}_{代码}_{日期}.json` / `em_{代码}_quote_{日期}.json`），并追加索引行到
   `_archive/_index.jsonl`。peers 批量/AH 对照额外落盘的 `_em_*.json`（fill 未引用者）
   由你手动移入 `_archive/`，命名 `em_{代码或标签}_{日期}.json`——**任何落盘文件都须等
   渲染成功后再动，提前移动 = 防伪链断裂拒渲染**；重渲旧 fill 从 `_archive/` 取回原件。
   最后告知用户报告路径和质量分+估值分+时机分。
...
质量分、估值分、时机分、徽章色、加权列、质量分汇总（第 7 章）、三轨判词与「三轨判定与仓位结论」卡（第 12 章，时机判定小表由模型先写）全部由脚本生成——报告里的数字与文件名一致。

**fill 产出前自检清单**（写作侧自查流程工具——校验唯一权威是 render 报错（v4.11.2 起），本清单只承载"写错后返工成本高"的地档项；不达标渲染器直接拒绝、不生成报告）：

- [ ] `conclusion_html` 四卡齐全（.concl-grid 结论卡：关键优势/关键弱点/当前市场认知/核心投资逻辑，卡尾「详见 X 章」索引）且纯文本 ≥120 字（补充修订四下调——四卡 ul 短列表化后原 200 会误伤）
...
- [ ] 正文引用章节一律用编号/名称（第 4 章 / 4.4 / 第 6 章风险评估）；**禁止 L1/L3/L4/1D 等框架内部代号出现在正文**（只允许在评分表、P0 卡与 fill JSON 字段名里；渲染器检出会告警）
- [ ] 正文【…】只用于黄灯类别标注（【b …】，单个 a-d 字母开头）；其他用途一律用（）或「」——其余【…】会被判为未替换占位符而拒渲染
...
| `scripts/visual_check.py` | 渲染后视觉审计（`python` 运行，溢出/槽位折行/裁切双视口检查） | Report Output |
...
- [ ] 核心结论是否四卡齐全、**四卡统一 `<ul>` 短列表**（补充修订四：每条 ≤12 字短语 + 至多一个括注（数字/日期/来源 ≤15 字））：①关键优势恰 3 条壁垒短语（括注带数字锚） ②关键弱点 2-3 条不回避（括注带数据） ③当前市场认知恰 2 条（卖方假设带来源日期 + 定价隐含假设） ④核心投资逻辑恰 3 条固定槽位（论点 / 可观测证伪条件 / 路径判断）？每卡是否带「详见 X 章」索引？是否**无评分表/无判词卡/无三块评分卡**（评分数字已在 Hero 与 section-meta）？
- [ ] L4 形态硬门禁（v4.10）：l4_html 是否含 `.pm-grid` 损失预演三联卡？黄灯扣分明细表行数是否与 `yellow_deductions` 逐条配平（缺卡/不配平即拒渲染）？
- [ ] 4.3 护城河是否用 trig-strip 清单列示（2-4 条：名称 + 一句论据 + 评级 宽阔/中等/狭窄，hit/pending/miss 三色）？5.3 催化剂是否同款清单（事件 + 时间窗 + 影响，已发生/进行中/已证伪）？5.2 多项目标的是否考虑同款清单（项目 + 产能/投产时间 + 进度；单项目可整段论证）？清单块 `<p>` 是否 ≤2（判词+评分段，渲染器超限告警）？
- [ ] 4.3 新三约束是否落实（v4.10.1）：定价权类论据带分域可观测判据（B2B/2C/金融，无判据按空话顶格 5.9，禁跨域套用）？边际改善类论据（毛利率回升/不良下降/份额提升）带同步性检验 α/β 结论（变窄判据同样过检验）？评分末拍竞争压力测试四件套齐全（场景/判断/先例/阈值，阈值在 14 章 triggers/dash 有对应行）？周期股毛利率归因三方向（产品端/价差/成本端）已做？
...
- [ ] v4.11.3 三卡字段是否已填：`drivers`/`driver_verdict`（P0 驱动卡；**v5.1.1 起第一变量由脚本按 sensitivity impact 降序加冕**——降序首行须有同名驱动卡否则拒渲染，first_var 仅缺 sensitivity 时回退、恰 1 个）、`cycle_stages`（周期章，driver 显示宽 ≤48、current 恰 1 个）、`dcf`（稳健成长/稳定价值强制，verdict ≥40 字、现价比价不手写由脚本算）？三种旧手写写法（p0_html「为什么…」info-card / cycle_html 阶段表 / valuation_html DCF 表）是否已清除？
- [ ] v5.1.2 一致性校验是否已过：手写表四禁（p0 敏感性表 / valuation 三情景表与指标卡 / l1 多年年表 / gap 逐机构对照表——字段或脚本已承载则删手写）？红旗 ≥2 项时 conclusion 首卡含「利润真实性存疑」？thesis 三价在 scenario span 内（裸文本绕行拒渲染）？时机小表得分与 `timing_scores`、技术面信号与落盘 `timing` 一致？triggers 的 hit/miss 行 target 写「阈值｜实际值」且方向不矛盾？4.3 压力测试阈值在 14 章 triggers/dash 有对应行？gap_plot 净利维度 consensus 照抄落盘、ours 与基础情景一致？
- [ ] 港股/AH 比价是否用了税后股息？税率是否标"以最新法规为准"？A+H/红筹标的写每股股息时是否注明宣派币种与 A 股折算口径（v4.11.1：如「公司以港元宣派，A 股按汇率折算人民币派发」），股息率分子分母同币种同源？
- [ ] 周期股结论是否同时覆盖商品价格假设与产能/产量轨迹（v4.11.1）？「看量不看价」仅限 3A 增长验证归因语境，不得外推为 thesis 级判断？
- [ ] 每个关键数字是否有来源或标"估算"？缺失是否写"未获取到"？
- [ ] 现价/PE(TTM) 是否来自 `em_fetch --out` 落盘、fill 已回填 `quote.source_file`？（神华 601088 事故后立规：现价类数字与估值分四件套同级防伪，渲染器比对落盘值——price/pe_ttm 偏差 >1% 拒渲染；v4.8 起 `valuation_inputs.pe_ttm` 同级比对、`pe_band` 与落盘历史带完全无交集拒、`risk_free`/`div_yield` 偏差 >0.3pct 告警；**v4.11.1 起 date ≥ 2026-09-02 的报告缺 quote 字段本身即拒渲染**）；**时机判定小表的技术面信号（现价/MA60/MA120/52 周高低）是否取自落盘 `timing` 字段而非手估**？
- [ ] 三情景是否围绕 Phase 0 驱动？三指标（中枢/赔率/离散度）是否脚本算？
- [ ] 预期差是否到档位（A/B/C），卖方假设是否带来源？落 B 档的标的一致预期覆盖 ≥5 家时，是否已先检索研报（禁直接反推）？v4.11.1 起 `gap_plot` 分布图：机构点名与数值是否照抄 E5「净利明细/目标价明细」行（禁编造/禁手估，同机构取 180 天内最新）？共同披露维度是否 ≥3 家才进图、不足维度是否落 text_dims 附注？本文假设点与 valuation 三情景中枢是否口径一致（渲染器对目标价行相对偏差 >2% 且绝对差 >0.1 告警）？`<!--GAP-->` 锚点是否写在 gap_html 章首？
- [ ] v4.9 必填图字段是否已填：`fin_trend`（4.4 财务趋势图墙）+ 锚点 `<!--FIN_TREND-->` 写在 4.4 维度块内、`growth_plot`（5.1 利润增长图）+ 锚点 `<!--GROWTH-->` 写在 5.1 维度块首——缺失即拒渲染（结构性下限同样拒：fin_trend 年数 ≥3 且有效面板 ≥3、growth_plot hist 有效年 ≥3；`growth_plot` 未盈利/管线分型豁免；数据分别照抄 E3 年表与 E5 一致预期，禁手写年表/手估）？`peers_plot` 是否已填（缺失触发渲染器告警，消解方式见「报告产出格式」第 2 步）？目标点与同业是否同口径（A/H、IFRS/经调整）？可选图字段是否已按数据可得性填写（缺失对应图不生成、不告警）：`sensitivity`（2 章龙卷风）；`segments`（4.1 业务构成，E6 回填、**利润口径=毛利**）+ 锚点 `<!--SEGMENTS-->` 是否写在 4.1 维度块内；`industry_chain`（4.2 产业链，只列行业不列企业）+ 锚点 `<!--CHAIN-->`；`consensus`（8 章走廊卖方目标价带，E5 回填）；`triggers`（14 章触发条件状态条，status 限 hit/miss/pending、宜 3-6 条）；`holders`（12 章户数趋势，v4.11.0 起照抄 E4「季度序列」行 ≥3 期）；`pe_history` 是否按 v4.7.1 分型规则处理（周期/稳健成长/稳定价值/困境反转 → 第 11 章必写 + 本字段按数据可得性填，含 `milestones` 与 v4.8 `p25`/`p75`（E1 落盘直接引用，禁手估）；快速成长/未盈利 → 禁填）？`price_history` 是否同规则处理（v4.11.0 起照抄 E2「全序列」行的 open/high/low/close/pe 四值+月末 PE（`YYYY-MM=开,高,低,收[/PE]`；四值为 ? 的月只写 close/pe），季K 蜡烛由脚本聚合、亏损期底纹与 PE 截断自动处理；`cycle_html` 缺失时两图无处显示会告警）？
- [ ] 回测模式：`prev` 与 `review_html` 是否成对？复盘是否只进 R 章节？`triggers` 是否已承载旧触发条件核对结果（status=hit/miss/pending，metric 写「阈值｜实际值」）？`dash_html` 是否已不再手写旧触发条件核对表（v4.9.1 起废止，由状态条承载）？
- [ ] 渲染后是否无 `{{}}`/`【】` 残留、无缺字段警告？
...
- **妙想 MCP 与 mx-* skill 双缺席必须显式标注「已走降级路径」，禁止静默跳过**（硬门禁与标注语句见 Phase 1「工具检查硬门禁」，缺席未标注 = 违反降级纪律）
