...
2. **渲染**：
   ...
   **估值分按 `valuation_inputs` 四件套脚本强制计算，fill 手填值一律被覆盖**）、
   ...
   回测哑铃 `prev` 按可选字段存在性生成，缺失静默跳过；PE 历史带 `pe_history` 生成后挂在
   第 10 章章首——`cycle_html` 缺失时整章连同历史带一起消失，渲染器告警）、占位符替换、
...
- [ ] 现价/PE(TTM) 是否来自 `em_fetch --out` 落盘、fill 已回填 `quote.source_file`？（神华 601088 事故后立规：现价类数字与估值分四件套同级防伪，渲染器比对落盘值，偏差 >1% 拒渲染）
- [ ] 三情景是否围绕 Phase 0 驱动？三指标（中枢/赔率/离散度）是否脚本算？
- [ ] 预期差是否到档位（A/B/C），卖方假设是否带来源？落 B 档的标的一致预期覆盖 ≥5 家时，是否已先检索研报（禁直接反推）？
- [ ] `peers_plot` 是否已填（缺失则图静默跳过）？目标点与同业是否同口径（A/H、IFRS/经调整）？可选图字段 `sensitivity` 是否已按数据可得性填写（缺失对应图不生成、不告警）？`pe_history` 是否按 v4.7.1 分型规则处理（周期/稳健成长/稳定价值/困境反转 → 第 10 章必写 + 本字段按数据可得性填，含 `milestones` 关键时点；快速成长/未盈利 → 禁填；`cycle_html` 缺失时图无处显示会告警）？
